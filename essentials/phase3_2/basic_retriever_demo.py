"""
Basic Scientific Retriever Demo

This script demonstrates the scientific retrieval capabilities of the RAG system
using spaCy embeddings to avoid huggingface_hub dependency issues.
"""
...
logger.info("Starting basic scientific retriever demo...")
...
def create_vector_store(chunks: List[Chunk], model: BasicEmbedding, output_dir: str) -> VectorStore:
...
def get_chunk_text(result: Dict) -> str:
    """Extract text content from result."""
    if "text" in result.get("metadata", {}):
        ...
        if "section" in metadata:
            text_parts.append(f"{metadata['section'].capitalize()}: ")
        
        if "paper_id" in metadata:
            text_parts.append(f"[{metadata['paper_id']}] ")
            
        if "source" in metadata:
            text_parts.append(f"Source: {metadata['source']}")
...
def run_retrieval_demo(query: str, vector_store: VectorStore, model: BasicEmbedding, verbose: bool = False):
    """Run retrieval demo.
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description="Basic Scientific Retriever Demo")
        ...
        logger.info(f"Arguments: data={args.data}, model={args.model}, vector_store={args.vector_store}, query={args.query}, verbose={args.verbose}")
        
        # Create embedding model
        logger.info(f"Loading spaCy model: {args.model}")
        model = BasicEmbedding(model_name=args.model)
        ...
        # Run retrieval demo
        run_retrieval_demo(args.query, vector_store, model, args.verbose)
    except Exception as e:
